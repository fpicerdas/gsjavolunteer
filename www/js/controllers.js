angular.module("gsja_volunteer_app.controllers", [])
...
.controller("indexCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture,$translate,tmhDynamicLocale){
	...
	popover_template += "			<a  class=\"item dark-ink\" ng-href=\"#/gsja_volunteer_app/about_us\" ng-click=\"popover.hide()\">";
	...
	popover_template += "			</a>";
	popover_template += "			<a  class=\"item dark-ink\" ng-href=\"#/gsja_volunteer_app/form_input_data_barang\" ng-click=\"popover.hide()\">";
	popover_template += "			{{ 'Input Data Barang' | translate }}";
	popover_template += "			</a>";
	popover_template += "			<a  class=\"item dark-ink\" ng-href=\"#/gsja_volunteer_app/form_add_new_inventory\" ng-click=\"popover.hide()\">";
	popover_template += "			{{ 'Add New Inventory' | translate }}";
	popover_template += "			</a>";
	popover_template += "		</ion-list>";
	popover_template += "	</ion-content>";
...
// TODO: form_add_new_inventoryCtrl --|-- 
.controller("form_add_new_inventoryCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture,$translate,tmhDynamicLocale){
	...
	// TODO: form_add_new_inventoryCtrl --|-- $scope.scrollTop
	$rootScope.scrollTop = function(){
		$timeout(function(){
			$ionicScrollDelegate.$getByHandle("top").scrollTop();
		},100);
	};
	// TODO: form_add_new_inventoryCtrl --|-- $scope.toggleGroup
	$scope.toggleGroup = function(group) {
		if ($scope.isGroupShown(group)) {
			$scope.shownGroup = null;
		} else {
	...
	// TODO: form_add_new_inventoryCtrl --|-- $scope.redirect
	// redirect
	$scope.redirect = function($url){
		$window.location.href = $url;
	};
	...
	// Form Request
	//add_new_inventory
	$scope.form_add_new_inventory= {};
	// TODO: form_add_new_inventoryCtrl --|-- $scope.submitAdd_new_inventory
	$scope.submitAdd_new_inventory = function(){
		// animation loading 
		$ionicLoading.show();
			...
			.then(function(response) {
				...
				$timeout(function() {
					...
					var alertPopup = $ionicPopup.alert({
						...
						// clear input
						$scope.form_add_new_inventory.nama_barang = "";
						$scope.form_add_new_inventory.kondisi_posisi = "";
						$scope.form_add_new_inventory.foto_barang = "";
					}
			}, 500);
		});
	};
	...
	// TODO: form_add_new_inventoryCtrl --|-- controller_by_user
	// controller by user 
	function controller_by_user(){
		try {
			...
			console.log("%cerror: %cPage: `form_add_new_inventory` and field: `Custom Controller`","color:blue;font-size:18px","color:red;font-size:18px");
			console.dir(e);
		}
	}
	$scope.rating = {};
...
// TODO: list_barangCtrl --|-- 
.controller("list_barangCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture,$translate,tmhDynamicLocale){
	...
	$rootScope.grid128 = parseInt($rootScope.ionWidth / 128) ;
	$rootScope.grid256 = parseInt($rootScope.ionWidth / 256) ;
	$rootScope.last_edit = "table (list_barang)" ;
	$scope.$on("$ionicView.afterEnter", function (){
		var page_id = $state.current.name ;
		$rootScope.page_id = page_id.replace(".","-") ;
	});
	...
	// TODO: list_barangCtrl --|-- $scope.scrollTop
	$rootScope.scrollTop = function(){
		$timeout(function(){
			$ionicScrollDelegate.$getByHandle("top").scrollTop();
		},100);
	};
	// TODO: list_barangCtrl --|-- $scope.toggleGroup
	$scope.toggleGroup = function(group) {
		if ($scope.isGroupShown(group)) {
			$scope.shownGroup = null;
		} else {
	...
	// TODO: list_barangCtrl --|-- $scope.redirect
	// redirect
	$scope.redirect = function($url){
		$window.location.href = $url;
	};
	...
	// TODO: list_barangCtrl --|-- $scope.showAuthentication
	...
	//fix url List Barang
	targetQuery = "json=list_barang"; //default param
	raplaceWithQuery = "json=list_barang";
	
	
	// TODO: list_barangCtrl --|-- $scope.splitArray
	...
	var lastPush = 0;
	var data_list_barangs = [];
	
	localforage.getItem("data_list_barangs_" + $scope.hashURL, function(err, get_list_barangs){
		if(get_list_barangs === null){
			data_list_barangs =[];
		}else{
			data_list_barangs = JSON.parse(get_list_barangs);
			$scope.data_list_barangs =JSON.parse( get_list_barangs);
			$scope.list_barangs = [];
			for(lastPush = 0; lastPush < 50; lastPush++) {
				if (angular.isObject(data_list_barangs[lastPush])){
					$scope.list_barangs.push(data_list_barangs[lastPush]);
				};
			}
			$timeout(function() {
				$ionicLoading.hide();
	...
	if(data_list_barangs === null ){
		data_list_barangs =[];
	}
	if(data_list_barangs.length === 0 ){
		$timeout(function() {
			...
			// TODO: list_barangCtrl --|-- $http.get
			$http.get(url_request,http_header).then(function(response) {
				data_list_barangs = response.data;
				$scope.data_list_barangs = response.data;
				// TODO: list_barangCtrl --|---------- set:localforage
				localforage.setItem("data_list_barangs_" + $scope.hashURL, JSON.stringify(data_list_barangs));
				$scope.list_barangs = [];
				for(lastPush = 0; lastPush < 50; lastPush++) {
					if (angular.isObject(data_list_barangs[lastPush])){
						$scope.list_barangs.push(data_list_barangs[lastPush]);
				...
				$timeout(function() {
					...
					// TODO: list_barangCtrl --|------ $http.jsonp
					$http.jsonp(url_request,http_header).success(function(data){
						data_list_barangs = data;
						$scope.data_list_barangs = data;
						$ionicLoading.hide();
						// TODO: list_barangCtrl --|---------- set:localforage
						localforage.setItem("data_list_barangs_" + $scope.hashURL,JSON.stringify(data_list_barangs));
						controller_by_user();
					...
					if(response.status ===401){
						// TODO: list_barangCtrl --|------------ error:Unauthorized
						$scope.showAuthentication();
					}else{
						// TODO: list_barangCtrl --|------------ error:Message
			...
			$timeout(function() {
				$ionicLoading.hide();
				controller_by_user();
				if(angular.isDefined($scope.data_list_barangs.data)){
					if($scope.data_list_barangs.data.status ===401){
						$scope.showAuthentication();
						return false;
					}
				}
	...
	// TODO: list_barangCtrl --|-- $scope.doRefresh
	...
	// TODO: list_barangCtrl --|-- controller_by_user
	// controller by user 
	function controller_by_user(){
		try {
			
//debug: all data
//console.log(data_list_barangs);
$ionicConfig.backButton.text("");
			
		} catch(e){
			console.log("%cerror: %cPage: `list_barang` and field: `Custom Controller`","color:blue;font-size:18px","color:red;font-size:18px");
			console.dir(e);
		}
	}
	$scope.rating = {};
...
// TODO: list_barang_singlesCtrl --|-- 
.controller("list_barang_singlesCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture,$translate,tmhDynamicLocale){
	...
	// TODO: list_barang_singlesCtrl --|-- $scope.scrollTop
	$rootScope.scrollTop = function(){
		$timeout(function(){
			$ionicScrollDelegate.$getByHandle("top").scrollTop();
		},100);
	};
	// TODO: list_barang_singlesCtrl --|-- $scope.toggleGroup
	$scope.toggleGroup = function(group) {
		if ($scope.isGroupShown(group)) {
			$scope.shownGroup = null;
		} else {
	...
	// TODO: list_barang_singlesCtrl --|-- $scope.redirect
	// redirect
	$scope.redirect = function($url){
		$window.location.href = $url;
	};
	...
	// TODO: list_barang_singlesCtrl --|-- $scope.showAuthentication
	$scope.showAuthentication  = function(){
		var authPopup = $ionicPopup.show({
			template: ' This page required login',
			title: "Authorization",
	...
	var current_item = [];
	localforage.getItem("data_list_barang_single_" + $scope.hashURL, function(err, get_datas){
		if(get_datas === null){
			current_item = [];
		}else{
			if(get_datas !== null){
				...
				$timeout(function(){
					$ionicLoading.hide();
					$scope.list_barang = current_item ;
					controller_by_user();
				}, 500);
			};
		};
	...
	if( current_item.length === 0 ){
		...
		// TODO: list_barang_singlesCtrl --|-- $http.get
		$http.get($scope.fetchURL,http_header).then(function(response) {
			// Get data single
			var datas = response.data;
			// TODO: list_barang_singlesCtrl --|---------- set:localforage
			localforage.setItem("data_list_barang_single_" + $scope.hashURL,JSON.stringify(datas));
			current_item = datas ;
		},function(data) {
					// Error message
					var alertPopup = $ionicPopup.alert({
		...
		// TODO: list_barang_singlesCtrl --|-- $scope.doRefresh
	$scope.doRefresh = function(){
		// Retrieving data
		var itemID = $stateParams.id;
		var current_item = [];
		...
		$http.get($scope.fetchURL,http_header).then(function(response) {
			...
			// Error message
		// TODO: list_barang_singlesCtrl --|---------- $http.jsonp
	...
	// TODO: list_barang_singlesCtrl --|-- controller_by_user
	// controller by user 
	function controller_by_user(){
		try {
...
$ionicConfig.backButton.text("");			
			...
			console.log("%cerror: %cPage: `list_barang_singles` and field: `Custom Controller`","color:blue;font-size:18px","color:red;font-size:18px");
			console.dir(e);
		}
	}
	$scope.rating = {};
...
// TODO: pttCtrl --|-- 
.controller("pttCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture,$translate,tmhDynamicLocale){
	...
	// TODO: pttCtrl --|-- $scope.scrollTop
	$rootScope.scrollTop = function(){
		$timeout(function(){
			$ionicScrollDelegate.$getByHandle("top").scrollTop();
		},100);
	};
	// TODO: pttCtrl --|-- $scope.toggleGroup
	$scope.toggleGroup = function(group) {
		if ($scope.isGroupShown(group)) {
			$scope.shownGroup = null;
		} else {
	...
	// TODO: pttCtrl --|-- controller_by_user
	// controller by user 
	function controller_by_user(){
		try {
...
$ionicConfig.backButton.text("");			
			...
			console.log("%cerror: %cPage: `ptt` and field: `Custom Controller`","color:blue;font-size:18px","color:red;font-size:18px");
			console.dir(e);
		}
	}
	$scope.rating = {};
